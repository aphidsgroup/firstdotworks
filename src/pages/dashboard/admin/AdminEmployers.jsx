...
import { Search, Eye, Edit, Building2, Activity, Filter, X, CheckCircle, Plus, Trash2, Download, CheckSquare, Square, ShieldCheck } from 'lucide-react'
...
function RegisterEntityModal({ onClose, emp = null }) {
  const [form, setForm] = useState(emp ? {
    ...
    scale: emp.size,
    sector: `${emp.city}, ${emp.state}`,
    contact: emp.contactPerson
  } : { name: '', industry: '', scale: '50-200', sector: '', contact: '' })
      ...
      <div className="bg-white dark:bg-[#0B0F19] border border-gray-100 dark:border-gray-800 rounded-2xl shadow-glow-cyan w-full max-w-xl flex flex-col relative overflow-hidden animate-slide-up">
        <div className="flex items-center justify-between p-6 border-b border-gray-100 dark:border-gray-800 bg-white/50 dark:bg-dark-bg/50 backdrop-blur-md">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-display font-bold text-brand-charcoal dark:text-white">{emp ? 'Modify Partner Entity' : 'Register Partner Entity'}</h2>
          </div>
        ...
        <div className="p-6">
          {success ? (
            <div className="text-center py-10">
              ...
              <h3 className="text-xl font-bold dark:text-white mb-2">{emp ? 'Entity Synchronized!' : 'Entity Initialized!'}</h3>
              <p className="text-sm text-gray-400">Partner node has been successfully {emp ? 'updated and re-synced' : 'registered'} in the network.</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid sm:grid-cols-2 gap-4">
                <div className="sm:col-span-2">
                  ...
                  <input required type="text" placeholder="e.g. Global Logic Systems" className="input bg-gray-50 dark:bg-dark-surface border-gray-200 dark:border-gray-800" value={form.name} onChange={e => setForm({...form, name: e.target.value})} />
                </div>
                <div>
                  <label className="text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-2 block">Industry Vertical</label>
                  <input type="text" placeholder="Information Technology" className="input bg-gray-50 dark:bg-dark-surface border-gray-200 dark:border-gray-800" value={form.industry} onChange={e => setForm({...form, industry: e.target.value})} />
                </div>
                <div>
                  <label className="text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-2 block">Operational Scale</label>
                  <select className="select bg-gray-50 dark:bg-dark-surface border-gray-200 dark:border-gray-800" value={form.scale} onChange={e => setForm({...form, scale: e.target.value})}>
                ...
                <div>
                  ...
                  <input type="text" placeholder="Chennai, TN" className="input bg-gray-50 dark:bg-dark-surface border-gray-200 dark:border-gray-800" value={form.sector} onChange={e => setForm({...form, sector: e.target.value})} />
                </div>
                <div>
                  <label className="text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-2 block">Lead Contact</label>
                  <input type="text" placeholder="Name of POC" className="input bg-gray-50 dark:bg-dark-surface border-gray-200 dark:border-gray-800" value={form.contact} onChange={e => setForm({...form, contact: e.target.value})} />
                </div>
              </div>
...
export default function AdminEmployers() {
  ...
  const handleBulkExport = () => {
    const selectedData = employers.filter(e => selectedIds.includes(e.id))
    const headers = EMPLOYER_COLUMNS.map(col => col.label).join(',')
    const rows = selectedData.map(e => 
      EMPLOYER_COLUMNS.map(col => {
        const val = typeof col.accessor === 'function' ? col.accessor(e) : e[col.accessor]
        return `"${val}"`
      }).join(',')
    ).join('\n')
  ...
  const handleBulkDelete = () => {
    if (window.confirm(`Deactivate ${selectedIds.length} partner entities?`)) {
      alert(`Simulated deactivation of Entity IDs: ${selectedIds.join(', ')}`)
      setSelectedIds([])
    }
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-8">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        {showModal && <RegisterEntityModal onClose={() => { setShowModal(false); setSelectedEmp(null); }} emp={selectedEmp} />}
        <div>
          ...
          <table className="table w-full">
            <thead className="bg-gray-50 dark:bg-dark-bg border-b border-gray-100 dark:border-gray-800/50">
              <tr>
                ...
                {['Entity Profile', 'Sector Division', 'Location', 'Operations Contact', 'Active Nodes', 'Deployed', 'Status', 'Execute'].map(h => <th key={h} className="th px-6 py-3 text-[10px] font-bold uppercase tracking-widest text-gray-400">{h}</th>)}
              </tr>
            </thead>
            <tbody className="bg-white dark:bg-dark-surface divide-y divide-gray-100 dark:divide-gray-800/50">
              {filtered.map(emp => (
                <tr key={emp.id} className={`tr-hover transition-colors ${selectedIds.includes(emp.id) ? 'bg-brand-cyan/5' : 'hover:bg-gray-50 dark:hover:bg-dark-bg'}`}>
                  <td className="td px-6 py-4">
                    <button onClick={(e) => toggleSelect(emp.id, e)} className={`${selectedIds.includes(emp.id) ? 'text-brand-cyan' : 'text-gray-300 dark:text-gray-600'} hover:text-brand-cyan transition-colors`}>
                  ...
                  <td className="td px-6 py-4">
                    <div className="flex items-center gap-1">
                      <button onClick={() => handleEdit(emp)} className="p-2 rounded-lg hover:bg-brand-cyan/10 text-gray-400 hover:text-brand-cyan transition-colors" title="Inspect"><Eye size={16} /></button>
                      <button onClick={() => handleEdit(emp)} className="p-2 rounded-lg hover:bg-brand-orange/10 text-gray-400 hover:text-brand-orange transition-colors" title="Modify"><Edit size={16} /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {selectedIds.length > 0 && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-brand-charcoal dark:bg-white text-white dark:text-brand-charcoal px-6 py-4 rounded-2xl shadow-2xl z-50 flex items-center gap-6 animate-slide-up border border-white/10 dark:border-brand-charcoal/10">
          <div className="flex items-center gap-3 pr-6 border-r border-white/20 dark:border-brand-charcoal/20">
            <span className="text-sm font-bold">{selectedIds.length} partners selected</span>
            ...
            <div className="relative group">
              ...
              <div className="absolute bottom-full left-0 mb-2 hidden group-hover:block w-48 bg-white dark:bg-dark-surface border border-gray-100 dark:border-gray-800 rounded-xl shadow-xl p-2 text-brand-charcoal dark:text-white">
                 {['active', 'inactive', 'pending'].map(s => (
                   <button key={s} onClick={() => { alert(`Bulk marked ${selectedIds.length} as ${s}`); setSelectedIds([]) }} className="w-full text-left px-3 py-2 hover:bg-gray-50 dark:hover:bg-white/5 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-colors">{s}</button>
                 ))}
              </div>
            </div>
